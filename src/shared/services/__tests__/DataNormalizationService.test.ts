...
describe('DataNormalizationService', () => {
  it('should normalize report data (happy path)', () => {
    const raw = [
  ...
  it('should normalize report with missing/invalid values falls back to 0 or numeric parse', () => {
    const raw = [
  ...
  it('should normalize json with flexible key names', () => {
    const raw = [
  ...
  it('should normalize api_v2 mapping', () => {
    const raw = [
  ...
  it('should handle empty data arrays', () => {
    ...
    const ds = DataNormalizationService.normalize(raw, DataSourceType.REPORT);
  ...
  it('should handle invalid data types gracefully', () => {
    const raw = [
      { date: '2025-01-15', value: 'invalid_number' },
      { date: '2025-01-16', value: {} },
      { date: '2025-01-17', value: [] }
    ];

    const ds = DataNormalizationService.normalize(raw as Record<string, unknown>[], DataSourceType.REPORT);
    expect(ds.data.length).toBe(3);
    expect(ds.data[0].value).toBeNaN();
    expect(ds.data[1].value).toBeNaN();
    expect(ds.data[2].value).toBeNaN();
  });
});