...
export const AdminAnalytics: React.FC = () => {
  ...
  const fetchSystemSettings = useSystemSettingsStore((state) => state.fetchSettings);
  ...
  const averageStationsPerUser = useMemo(() => {
    ...
    const totalAssigned = users.reduce((total, user) => total + (user.assigned_stations?.length || 0), 0);
    return totalAssigned / users.length;
  }, [users]);

  const criticalStations = useMemo(() =>
    stations.filter((station) => station.status === 'maintenance' || station.status === 'inactive'),
  [stations]);
  ...
  const totalAdmins = userRoleData.find((item) => item.role === 'Administrador')?.count ?? 0;
  const totalTechnicians = userRoleData.find((item) => item.role === 'Técnico')?.count ?? 0;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Métricas clave del uso de la plataforma, salud operacional y actividad reciente.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">
                  {usersLoading ? <SkeletonBlock className="h-7 w-16" /> : totalUsers}
                </h3>
              </div>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Estaciones monitoreadas</p>
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">
                  {stationsLoading ? <SkeletonBlock className="h-7 w-16" /> : totalStations}
                </h3>
              </div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-3">
              {criticalStations.length} estación(es) requiere(n) atención técnica
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Promedio asignaciones</p>
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mt-1">
                  {usersLoading ? <SkeletonBlock className="h-7 w-16" /> : averageStationsPerUser.toFixed(1)}
                </h3>
              </div>
        ...
        <Card>
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Último respaldo</p>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
            <div>
              ...
              <CardDescription>Resumen de usuarios por perfil de acceso</CardDescription>
            </div>
          ...
          <CardContent>
            {usersLoading ? (
                  ...
                  return (
                    <div key={role} className="space-y-1">
                      ...
                      <div className="h-3 rounded-full bg-gray-200 dark:bg-gray-800 overflow-hidden">
                        <div
                          className={`h-full ${ROLE_COLORS[role] ?? 'bg-blue-500'} transition-all`}
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">No hay datos de usuarios disponibles.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
            <div>
              ...
              <CardDescription>Monitoreo operativo por estado actual</CardDescription>
            </div>
            ...
            {stationsLoading ? (
              ...
              <div className="space-y-3">
                {stationStatusData.map(({ status, label, count }) => {
                  const percentage = totalStations ? Math.round((count / totalStations) * 100) : 0;
                  return (
                    <div key={status} className="space-y-1">
                      ...
                      <div className="h-3 rounded-full bg-gray-200 dark:bg-gray-800 overflow-hidden">
                        <div
                          className={`h-full ${STATION_STATUS_COLORS[status] ?? 'bg-gray-400'} transition-all`}
                          style={{ width: `${Math.max(percentage, 4)}%` }}
                          aria-label={`${label}: ${percentage}%`}
                        />
                      </div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">No hay estaciones registradas.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Eventos clave registrados en el sistema</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {activityLoading ? (
              ...
              activityStats.recentActivity.map((log) => {
                const statusConfig = STATUS_LABELS[log.status] ?? STATUS_LABELS.info;
                return (
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">No hay actividad reciente registrada.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Usuarios con mayor cantidad de estaciones asignadas</CardDescription>
          </CardHeader>
          <CardContent>
            {usersLoading ? (
              <div className="space-y-3">
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                Distribución de eventos registrados por tipo y estado en el último periodo.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          {activityLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 p-4">
                <p className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Por tipo</p>
              ...
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 p-4">
                <p className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Por estado</p>
                <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                  {Object.entries(activityStats.byStatus).map(([status, count]) => {
                    const statusConfig = STATUS_LABELS[status] ?? STATUS_LABELS.info;
                    return (
